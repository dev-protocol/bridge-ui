//
// this is copied from https://github.com/OffchainLabs/arb-token-bridge/blob/master/packages/token-bridge-sdk/src/util/graph.ts
//
...
export const getNodes = async (networkID: string, minAfterSendCount = 0, offset = 0): Promise<NodeDataResult[]> => {
	...
	if (nodes.length === 0) {
		...
		return nodes.concat(await getNodes(networkID, minAfterSendCount, offset + nodes.length));
	}
};
...
const getLatestIndexedBlockNumber = async (subgraphName: string) => {
	try {
		const res = await axios.post('https://api.thegraph.com/index-node/graphql', {
